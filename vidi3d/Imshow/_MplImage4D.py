...
class _MplImage4D(QtCore.QObject):    
    def __init__(self, complexIm3, pixdim,interpolation, initLocation, imageType, parent=None):
        ...
        self.zslice = _zslice(parent=parent, complexImage=complexIm3[:,:,initLocation[2],initLocation[3]], location=initLocation[0:2], sliceNum=initLocation[2], maxSliceNum = complexIm3.shape[2], imageType=dd.ImageType.mag, locationLabels=labels, aspect=aspz, interpolation=interpolation) #dpi doesn't affect imshow...cause imshow rescales to figure size                
        ...
        self.yslice = _yslice(parent=parent,complexImage=complexIm3[:,initLocation[1],:,initLocation[3]], location=initLocation[0::2], sliceNum=initLocation[1], maxSliceNum = complexIm3.shape[1], imageType=dd.ImageType.mag, locationLabels=labels, aspect=aspy, interpolation=interpolation)
        ...
        self.xslice = _xslice(parent=parent,complexImage=complexIm3[initLocation[0],:,:,initLocation[3]].T, location=initLocation[2:0:-1], sliceNum=initLocation[0],maxSliceNum = complexIm3.shape[0],  imageType=dd.ImageType.mag, locationLabels=labels,  aspect=aspx, interpolation=interpolation)
        self.xsliceNav=NavigationToolbar(self.xslice,self.xslice)
        
        self.xplot=_MplPlot._MplPlot(complexIm3[:,initLocation[1],initLocation[2],initLocation[3]],title="X Plot",initMarkerPosn=initLocation[0])        
        self.yplot=_MplPlot._MplPlot(complexIm3[initLocation[0],:,initLocation[2],initLocation[3]],title="Y Plot",initMarkerPosn=initLocation[1])        
        self.zplot=_MplPlot._MplPlot(complexIm3[initLocation[0],initLocation[1],:,initLocation[3]],title="Z Plot",initMarkerPosn=initLocation[2])
        ...
        while tavg.ndim>1:
            tavg=tavg.mean(0)
        self.tplot=_MplPlot._MplPlot(tavg,title="T Plot",initMarkerPosn=initLocation[3]) 
    ...
    def onXChange(self,value):
        ...
        self.yplot.showComplexDataAndMarkersChange(self.raw[value,:,self.location[2],self.location[3]],self.location[1])
        self.zplot.showComplexDataAndMarkersChange(self.raw[value,self.location[1],:,self.location[3]],self.location[2])
    ...
    def onYChange(self,value):        
        ...
        self.xplot.showComplexDataAndMarkersChange(self.raw[:,value,self.location[2],self.location[3]],self.location[0])
        self.zplot.showComplexDataAndMarkersChange(self.raw[self.location[0],value,:,self.location[3]],self.location[2])
        tavg=self.raw[max(0,self.location[0]-self.tavgrad):min(self.location[0]+self.tavgrad+1,self.raw.shape[0]),
                max(0,self.location[1]-self.tavgrad):min(self.location[1]+self.tavgrad+1,self.raw.shape[1]),
                max(0,self.location[2]-self.tavgrad):min(self.location[2]+self.tavgrad+1,self.raw.shape[2]),:]
        while tavg.ndim>1:
    ...
    def onZChange(self,value):        
        ...
        self.xplot.showComplexDataAndMarkersChange(self.raw[:,self.location[1],value,self.location[3]],self.location[0])
        self.yplot.showComplexDataAndMarkersChange(self.raw[self.location[0],:,value,self.location[3]],self.location[1])
        tavg=self.raw[max(0,self.location[0]-self.tavgrad):min(self.location[0]+self.tavgrad+1,self.raw.shape[0]),
                max(0,self.location[1]-self.tavgrad):min(self.location[1]+self.tavgrad+1,self.raw.shape[1]),
                max(0,self.location[2]-self.tavgrad):min(self.location[2]+self.tavgrad+1,self.raw.shape[2]),:]
        while tavg.ndim>1:
    ...
    def onTChange(self,value):        
        ...
        self.xplot.showComplexDataAndMarkersChange(self.raw[:,self.location[1],self.location[2],self.location[3]],self.location[0])
        self.yplot.showComplexDataAndMarkersChange(self.raw[self.location[0],:,self.location[2],self.location[3]],self.location[1])
        self.zplot.showComplexDataAndMarkersChange(self.raw[self.location[0],self.location[1],:,self.location[3]],self.location[2])
        tavg=self.raw[max(0,self.location[0]-self.tavgrad):min(self.location[0]+self.tavgrad+1,self.raw.shape[0]),
                max(0,self.location[1]-self.tavgrad):min(self.location[1]+self.tavgrad+1,self.raw.shape[1]),
                max(0,self.location[2]-self.tavgrad):min(self.location[2]+self.tavgrad+1,self.raw.shape[2]),:]
        while tavg.ndim>1:
