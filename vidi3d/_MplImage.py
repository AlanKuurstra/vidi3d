...
class _MplImage(FigureCanvas):
    ...
    def __init__(self, complexImage, aspect='equal', overlay=None, parent=None, interpolation='none', origin = 'lower', imageType=None, windowLevel=None, location=None, locationLabels=None, colormap=None, overlayColormap=None ):        
        ...
        self.htxt=self.axes.text(-.5,location[1],currLabels[0]['textLabel'],bbox=dict(facecolor='white', alpha=0.7),va='center',ha='right')                 
        self.vline=self.axes.axvline(x=location[0],linewidth=1,color=currLabels[1]['color'])         
        self.vtxt=self.axes.text(location[0],-.5,currLabels[1]['textLabel'], bbox=dict(facecolor='white', alpha=0.7),va='top',ha='center') 
    ...
    def PressEvent(self,event): 
        ...
        if event.button==1:
            ...
            mpl.pyplot.figure()
            popOutPlot=mpl.pyplot.imshow(img,cmap=cmap,origin=origin, vmin=vmin,vmax=vmax, interpolation=interpolation)
